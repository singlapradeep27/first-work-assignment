...
export function validateForm(
  ...
  fields.forEach((field) => {
    ...
    if (field.required && (typeof value === "undefined" || !value?.length))
  ...
  if (
    ...
    validateSelectField(formData as SelectField)
  ) {
  ...
  if (formData.type === FieldType.NUMBER) {
    ...
    if (error) errors[formData.id] = error;
  }
...
export const validateSelectField = (field: SelectField) => {
  ...
  return (
    ...
    !field.options.filter((option) => option.value).length
  );
};
...
export const validateNumberField = (field: FormField) => {
  ...
  if (rule?.["min"] !== undefined && field?.defaultValue !== undefined) {
    if (Number(field?.defaultValue) < Number(rule?.["min"]))
      return "Default value cannot be lesser than min value";
  }
  if (rule?.["max"] !== undefined && field?.defaultValue !== undefined) {
    if (Number(field?.defaultValue) > Number(rule?.["max"]))
...
export const validateTextField = (field: FormField) => {
  ...
  if (rule?.["minLength"] !== undefined && field?.defaultValue !== undefined) {
    if ((field?.defaultValue as string)?.length < Number(rule?.["minLength"]))
      return "Default value length cannot be lesser than min value";
  }
  if (rule?.["max"] !== undefined && field?.defaultValue !== undefined) {
    if ((field?.defaultValue as string).length > Number(rule?.["maxLength"]))
      return "Default value length cannot be greater than max value";
  }
  return "";
};
